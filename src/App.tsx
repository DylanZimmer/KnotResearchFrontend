...
import type { FullNotation } from '../services/types'
...
function normalizeCrossingGroups(payload: unknown): CrossingGroup[] {
  ...
  return groups.map((group) => {
    ...
    if (
      typeof groupRecord?.numCrossings !== 'string' ||
      !Array.isArray(groupRecord.rolfIndexes) ||
      !groupRecord.rolfIndexes.every((rolfIndex) => typeof rolfIndex === 'string')
    ) {
      throw new Error('Unexpected crossing groups response')
    }

    return {
      numCrossings: groupRecord.numCrossings,
      rolfIndexes: groupRecord.rolfIndexes,
    }
  })
}

function getKnotNamesFromCrossingGroups(crossingGroups: CrossingGroup[]) {
  return crossingGroups.flatMap((group) =>
    group.rolfIndexes.map((rolfIndex) => `${group.numCrossings}_${rolfIndex}`),
  )
}
...
function App() {
  ...
  // Load the list of available knot crossing groups
  useEffect(() => {
    let cancelled = false

    async function loadKnotNames() {
      try {
        const res = await fetch(getApiUrl('/api/knots/crossing_groups'))
        const payload = await readJsonResponse(res)

        if (!res.ok) {
          throw new Error(payload?.error || 'Failed to load knot crossing groups')
        }

        const crossingGroups = normalizeCrossingGroups(payload)

        if (!cancelled) {
          setKnotNames(getKnotNamesFromCrossingGroups(crossingGroups))
        ...
        if (!cancelled) {
          ...
          setSelectedN2('')
          setKnotListError(
            error instanceof Error ? error.message : 'Failed to load knot crossing groups',
          )
        }
      } finally {
        if (!cancelled) {
