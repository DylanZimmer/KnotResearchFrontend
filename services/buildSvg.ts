  ...
  export function buildSvg({ vertex_positions, arrows, crossing_specs, handedness }: Geometry): string {
    console.log("handedness inside buildSvg : ", handedness)
    const W = 500, H = 500, MARGIN = 60, STROKE = 10;
    const GAP_STROKE = 18, GAP_HALF = 18, FONT = 13;
    const INNER_W = W - 2 * MARGIN, INNER_H = H - 2 * MARGIN;

    // Global orientation of the knot's traversal.
    // "R" reads the arrows array as-is; "L" reads it backwards, which is
    // equivalent to swapping [start, end] on every arrow before using it
    // for anything direction-dependent (i.e. the orientation arrows).
    // Geometric facts — positions, which strand is horizontal/vertical,
    // which strand is over/under at a crossing — are unaffected, since
    // none of those depend on which endpoint is "first".
    const reversed = handedness === "L";
    const orientArrow = (arrow: NumPair): NumPair =>
      reversed ? [arrow[1], arrow[0]] : arrow;
    ...
    for (const [crossing_id, under_line, over_line] of crossing_specs) {
      ...
      // ── Regular crossing ────────────────────────────────────────────────
      const underArrow = orientArrow(arrows[under_line]);
      const overArrow  = orientArrow(arrows[over_line]);
      const us = svgVerts[underArrow[0]], ue = svgVerts[underArrow[1]];
      const os = svgVerts[overArrow[0]],  oe = svgVerts[overArrow[1]];
